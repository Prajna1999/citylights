...
export default function Directory({ shops }: { shops: Shop[] }) {
  ...
  const filteredShops = useMemo(() => shops.filter((shop) => {
    ...
    const haystack = `${shop.name} ${shop.local} ${shop.category} ${shop.area} ${shop.owner ?? ""}`;
  ...
  return (
    <main className="site-shell">
      ...
      <section className="directory" id="browse">
        ...
        {view === "list" ? <div className="results-layout"><div className="results-column"><div className="results-header"><span>{filteredShops.length} places near you</span><span className="page-count">Page {safePage} of {totalPages}</span></div><div className="shop-list">{pagedShops.map((shop) => <article className="shop-card" key={shop.slug}><div className={`shop-thumb ${shop.color}`}>{shop.photoUrl ? <Image src={shop.photoUrl} alt="" fill sizes="72px" className="thumb-photo" /> : null}<span>{shop.initials}</span><i /></div><Link href={`/stores/${shop.slug}`} className="shop-info shop-link"><div className="shop-title"><div><h3>{shop.name}</h3><p className="local-name">{shop.local}</p></div><span className={isShut(shop) ? "status closed" : "status open"}><i />{statusText(shop)}</span></div><p className="shop-description">{shop.description}</p><div className="shop-meta"><span>{shop.category}</span><span>{shop.area}</span>{shop.distance ? <span>{shop.distance}</span> : null}</div></Link>{shop.phone ? <a href={telHref(shop.phone) ?? "#"} className="call-button" aria-label={`Call ${shop.name}`}><Icon name="phone" /></a> : <span className="call-button disabled" title="No number listed"><Icon name="phone" /></span>}</article>)}</div>{filteredShops.length === 0 && <div className="empty-state"><WheelMark className="empty-wheel" />No shops match that search.<button onClick={clearFilters}>Clear search</button></div>}{totalPages > 1 && <nav className="pagination" aria-label="Pages of results"><button className="page-btn" disabled={safePage <= 1} onClick={() => setPage(safePage - 1)}>← Prev</button><span className="page-status">Showing {(safePage - 1) * PER_PAGE + 1}–{Math.min(safePage * PER_PAGE, filteredShops.length)} of {filteredShops.length}</span><button className="page-btn" disabled={safePage >= totalPages} onClick={() => setPage(safePage + 1)}>Next →</button></nav>}</div></div> : <MapPreview shops={filteredShops} large />}
      </section>
